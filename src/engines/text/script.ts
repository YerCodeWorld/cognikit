		...
		return data.parts.map((part, index) => {
			...
			if (part.startsWith('<')) {
				...
				if (target) {
					...
					// Add grading class to the element
					if (gradingClass) {
						modifiedPart = modifiedPart.replace(/class="([^"]*)"/, `class="$1 ${gradingClass}"`);
						if (!modifiedPart.includes('class=')) {
							modifiedPart = modifiedPart.replace('<edu-input', `<edu-input class="${gradingClass}"`);
						}
					}
	...
	private renderClassification(data: TextEngineClassificationData): string {
		...
		const categoriesHtml = `
			<div class="categories">
				${categories.map(cat => `
					<button class="category-btn" data-category="${escapeHtml(cat)}">
						${escapeHtml(cat)}
					</button>
				`).join('')}
			</div>
		...
		return `${categoriesHtml}<div class="words">${wordsHtml}</div>`;
	}
