 ...
 * a key in assets.
 */
export function setUpChipDataScalable(key: string, chip: EduChipScalable, assets?: NormalizedAssets["assetsById"]): void {
...
/**
 * EduChipScalable - A scalable version of EduChip
 *
 * Features:
 * - Container-relative sizing using CSS Container Queries
 * - Respects 44px Interaction Floor (minimum touch target)
 * - Fluid typography with clamp()
 * - Em-based spacing (scales with font-size)
 * - Aspect ratio aware (3:1 default)
 * - No overflow allowed
 */
export class EduChipScalable extends HTMLElement {
...
if (!customElements.get("edu-chip-scalable")) customElements.define("edu-chip-scalable", EduChipScalable);
