...
export class SequentialFillBlanks extends BaseInteraction<TextEngineSequentialInteractionData> {
	...
	render(): void {
		...
		this.innerHTML = `
			...
			<div class="text-container">
				<div class="step-label">Step ${this._currentStep + 1} of ${this._slides.length}</div>
	...
	grade(): GradingResult {
		...
		const currentSlide = this._slides[this._currentStep];
		const currentResponse = this.getResponseForStep(this._currentStep);
		const currentGrading = getBlanksGradingState(currentSlide, currentResponse);
