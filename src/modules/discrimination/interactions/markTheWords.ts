...
export class MarkTheWords extends BaseInteraction<TextEngineBaseData> {
	...
	constructor(data: TextEngineBaseData, config: InteractionConfig) {
		...
		// Initialize progress tracking (total target words)
		const totalTargets = data.targets.reduce((sum, target) =>
			sum + (target.endPos - target.startPos + 1), 0
		);
		this.initializeProgress(totalTargets);
	}
	...
	render(): void {
		...
		this._$text.addEventListener('change', () => {
			this.updateProgressBasedOnSelection();
	...
	/**
	 * Update progress bar based on how many target words have been selected
	 */
	private updateProgressBasedOnSelection() {
		const userData = this._$text.getValue() as TextEngineBaseUserData;
		const selectedIndices = new Set(userData.selectedIndices ?? []);

		// Count how many target words are selected
		...
		this.setProgress(selectedTargetCount);
	}
	...
	grade() {
		...
		const gradingState = getHighlightGradingState(this.data, userData);

		// Convert target-based grading state to word-index-based for display
		const wordGradingState: Record<number, any> = {};
		for (let i = 0; i < this.data.targets.length; i++) {
			const target = this.data.targets[i];
			const state = gradingState[i];

			for (let j = target.startPos; j <= target.endPos; j++) {
				wordGradingState[j] = state;
			}
		}
